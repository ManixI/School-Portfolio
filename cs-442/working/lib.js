 ...
 * @returns {Mesh}
 */
function load_mesh_from_obj_text(gl, text) {
    let prog = gl.createProgram();
    let file_mesh = Mesh.from_obj_text(gl, prog, text );
    return file_mesh;
...
/**
 * set uniform data for sampler
 * @param {WebGLRenderingContext} gl
 * @param {WebGLProgram} program
 * @param {string} name
 * @param {int} texture
 */
function set_texture_sampler (gl, program, name, sampler) {
    const loc = gl.getUniformLocation(program, name);
...
// takes the number of turns and returns radian equivelent
function radians(turns) {
    return 2 * Math.PI * turns;
}


function load_tex(tex_source) {
    let image = new Image();
    // callback
